...
function mapApplicantToPartialMatricula(a: RawApplicant): Partial<MatriculaDoc> {
  const estudiante = {
    ...
    sexo: a?.sexo ?? a?.estudiante?.sexo ?? '',
    religion: a?.religion ?? a?.estudiante?.religion ?? '',
    edadAnios: a?.edadAnios ?? a?.estudiante?.edadAnios ?? '',
    edadMeses: a?.edadMeses ?? a?.estudiante?.edadMeses ?? '',
    direccion: a?.direccionResidencia ?? a?.direccion ?? a?.estudiante?.direccion ?? '',
    barrio: a?.barrioAspirante ?? a?.barrio ?? '',
    ciudad: a?.ciudad ?? '',
    ultimoGrado: a?.ultimoGrado ?? '',
